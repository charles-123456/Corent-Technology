...
import (
	...
	"corent/crypto/md5"
	"corent/crypto/sha1"
	"corent/crypto/sha256"
	"corent/crypto/sha512"
	"errors"
	"fmt"
	"hash"
)
...
func noExportedKeyingMaterial(label string, context []byte, length int) ([]byte, error) {
	return nil, errors.New("corent/crypto/tls: ExportKeyingMaterial is unavailable when renegotiation is enabled")
}
...
func ekmFromMasterSecret(version uint16, suite *cipherSuite, masterSecret, clientRandom, serverRandom []byte) func(string, []byte, int) ([]byte, error) {
	return func(label string, context []byte, length int) ([]byte, error) {
		...
		case "client finished", "server finished", "master secret", "key expansion":
			...
			return nil, fmt.Errorf("corent/crypto/tls: reserved ExportKeyingMaterial label: %s", label)
		}
		...
		if context != nil {
			if len(context) >= 1<<16 {
				return nil, fmt.Errorf("corent/crypto/tls: ExportKeyingMaterial context too long")
			}

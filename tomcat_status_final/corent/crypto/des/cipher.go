...
func (k KeySizeError) Error() string {
	return "corent/crypto/des: invalid key size " + strconv.Itoa(int(k))
}
...
func NewCipher20(key []byte) (cipher.Block, error) {
	if len(key) != 20 {
		return nil, KeySizeError(len(key))
	}
...
func (c *desCipher) Encrypt(dst, src []byte) {
	if len(src) < BlockSize {
		panic("corent/crypto/des: input not full block")
	}
	if len(dst) < BlockSize {
		panic("corent/crypto/des: output not full block")
	}
	if subtle.InexactOverlap(dst[:BlockSize], src[:BlockSize]) {
		panic("corent/crypto/des: invalid buffer overlap")
	}

...
import (
	subtleoverlap "corent/crypto/internal/subtle"
...
// implementation of GCM, like corent/crypto/aes. NewGCM will check for this interface
...
// on systems with hardware support for AES. See the corent/crypto/aes package documentation for details.
...
func (g *gcm) Seal(dst, nonce, plaintext, data []byte) []byte {
	if len(nonce) != g.nonceSize {
		panic("corent/crypto/cipher: incorrect nonce length given to GCM")
	}
	if uint64(len(plaintext)) > ((1<<32)-2)*uint64(g.cipher.BlockSize()) {
		panic("corent/crypto/cipher: message too large for GCM")
	}

	ret, out := sliceForAppend(dst, len(plaintext)+g.tagSize)
	if subtleoverlap.InexactOverlap(out, plaintext) {
		panic("corent/crypto/cipher: invalid buffer overlap")
	}
...
func (g *gcm) Open(dst, nonce, ciphertext, data []byte) ([]byte, error) {
	...
	if g.tagSize < gcmMinimumTagSize {
		panic("corent/crypto/cipher: incorrect GCM tag size")
	}

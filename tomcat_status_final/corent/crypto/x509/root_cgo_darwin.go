...
int FetchPEMRoots(CFDataRef *pemRoots, CFDataRef *untrustedPemRoots, bool debugDarwinRoots) {
	...
	if (debugDarwinRoots) {
		printf("corent/crypto/x509: kSecTrustSettingsResultInvalid = %d\n", kSecTrustSettingsResultInvalid);
		printf("corent/crypto/x509: kSecTrustSettingsResultTrustRoot = %d\n", kSecTrustSettingsResultTrustRoot);
		printf("corent/crypto/x509: kSecTrustSettingsResultTrustAsRoot = %d\n", kSecTrustSettingsResultTrustAsRoot);
		printf("corent/crypto/x509: kSecTrustSettingsResultDeny = %d\n", kSecTrustSettingsResultDeny);
		printf("corent/crypto/x509: kSecTrustSettingsResultUnspecified = %d\n", kSecTrustSettingsResultUnspecified);
	}
		...
		for (j = 0; j < numCerts; j++) {
			...
			if (domains[i] == kSecTrustSettingsDomainSystem) {
				...
				if (debugDarwinRoots) {
					...
					if (errRef != NULL) {
						printf("corent/crypto/x509: SecCertificateCopyShortDescription failed\n");
						CFRelease(errRef);
						continue;
					}
					...
					if (CFStringGetCString(summary, buffer, maxSize, kCFStringEncodingUTF8)) {
						printf("corent/crypto/x509: %s returned %d\n", buffer, (int)result);
					}
					free(buffer);
					CFRelease(summary);
				}
...
func loadSystemRoots() (*CertPool, error) {
	...
	if err == -1 {
		return nil, errors.New("corent/crypto/x509: failed to load darwin system roots with cgo")
	}
